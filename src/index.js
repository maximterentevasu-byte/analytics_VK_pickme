...
async function fetchVkGroup(groupId, token) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'User-Agent': 'vk-sheets-members-sync/1.0'
    }
  });
  ...
  if (data.error) {
    ...
    throw new Error(`VK API error for group "${groupId}": [${errorCode}] ${errorMsg}`);
  }

  const group = Array.isArray(data.response) ? data.response[0] : null;

  if (!group) {
    throw new Error(`VK response is empty for group "${groupId}"`);
  }
